...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String: NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        vpnReachability.startTracking { [weak self] status in
            ...
            self?.vpnAdapter.reconnect(afterTimeInterval: 2)
        }
...
extension PacketTunnelProvider: OpenVPNAdapterDelegate {
    ...
    func _updateEvent(_ event: OpenVPNAdapterEvent, openVPNAdapter: OpenVPNAdapter) {
        ...
        case .connected:
            toSave = "CONNECTED"
            UserDefaults(suiteName: groupIdentifier)?.setValue(formatter.string(from: Date.now), forKey: "connected_on")
        case .disconnected:
            toSave = "DISCONNECTED"
        case .connecting:
            toSave = "CONNECTING"
